...
export class SaComponent implements OnInit {
  ...
  getAll(){
    this._activitiesService.activities$.subscribe(
      (activities : Activity[])=>{
        this.allActivities = activities.filter(act => act.attributes.categorie_de_l_activites.data.map(x => x.id).includes(2))
    ...
    this._activitiesCategoriesService.categories$.subscribe(
      (categories : ActivityCategory[])=>{
        this.categories = categories.filter(cat =>
          cat.attributes.categorie_de_l_activites.data[0].id == 2
        )
      }
    )
  }
